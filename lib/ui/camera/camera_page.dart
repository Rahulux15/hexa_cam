...
import '../common/save_dialog.dart';
...
class _CameraPageState extends ConsumerState<CameraPage>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      ref.read(measurementModeProvider.notifier).state = true;
    });
  ...
  Future<void> _requestPermissionsAndInitCamera() async {
    try {
      ...
      debugPrint('Permission flow error: $error');
    } finally {
      await _initCamera();
    }
  }
  ...
  Future<void> _initCamera() async {
    ...
    try {
      ...
      if (_controller == null && mounted) {
        debugPrint('Failed to initialize any camera');
        setState(() {
          _isInitialized = false;
          _viewMode = CameraViewMode.defaultOpen;
          _showCameraSettings = false;
  ...
  Widget _buildLeftRail(bool isTablet) {
    final measurementEnabled = ref.watch(measurementModeProvider);
  ...
  Widget _buildCalibrationSection(bool isTablet) {
    final stored = ref.watch(calibrationProvider)[_selectedLens];
    final microCalibration = ref.watch(microscopeCalibrationProvider);
    final microEntry =
  ...
  void _handleSideAction(CameraSideAction action) {
    switch (action) {
      ...
      case CameraSideAction.status:
        ref.read(measurementModeProvider.notifier).update((state) => !state);
        _refreshAnnotationMeasurements();
        break;
              ...
              children: AppConstants.magnificationLevels.map((lens) {
                return GestureDetector(
                  onTap: () {
                    setState(() {
                      _selectedLens = lens;
                      final microEntry = ref
                          .read(microscopeCalibrationProvider)
                          .getCalibrationForMagnification(lens);
                      final stored = ref.read(calibrationProvider)[lens];
  ...
  void _onPanEnd(DragEndDetails details) {
    ...
    final measurement = ref.read(measurementModeProvider)
  ...
  Future<void> _startVideoRecording() async {
    ...
    try {
      ...
      debugPrint('Video start error: $error');
  ...
  Future<void> _stopVideoRecording() async {
    ...
    try {
      ...
      debugPrint('Video stop error: $error');
      if (!mounted) return;
      setState(() => _isRecording = false);
                ...
                ClipRRect(
                  ...
                  child: Container(
                    ...
                    child: isVideo
                        ...
                        : MediaImage(
                            source: kIsWeb ? filePath : 'file://$filePath',
                            fit: BoxFit.cover,
  ...
  Future<ImageData?> _persistMedia({
    ...
    try {
      ...
      late final String mediaSourcePath;
      ...
      if (kIsWeb) {
        ...
        bytes = await FileService.readBytes(mediaSourcePath);
      }
      await MediaDatabase.saveAsset(mediaId, bytes);

      if (exportToDevice) {
        if (kIsWeb) {
          await FileService.saveToDevice(bytes, preferredName);
        } else if (isVideo) {
      ...
      final image = ImageData(
        ...
        mediaId: mediaId,
        thumbnailId: isVideo ? null : mediaId,
        timestamp: DateTime.now().toIso8601String(),
        cameraSettings: _settings,
      ...
      ref.read(foldersProvider).addImage(widget.folderId, image);
      _showMessage(
        exportToDevice
      ...
      debugPrint('Persist media error: $error');
  ...
  String? _buildMeasurementForPoints(
    ...
    if (!ref.read(measurementModeProvider)) return null;
  ...
  List<Annotation> _syncedAnnotations() {
    final showMeasurements = ref.read(measurementModeProvider);
    return _annotations
        .map(
  ...
  StoredCalibration? _effectiveCalibrationForLens(String lens) {
    final microEntry = ref
        .read(microscopeCalibrationProvider)
        .getCalibrationForMagnification(lens);
    final storedCalibration = ref.read(calibrationProvider)[lens];
  ...
  String _buildStampLabel() {
    ...
    final stored = ref.read(calibrationProvider)[_selectedLens];
  ...
  void _saveCalibrationFromAnnotation(
    ...
    final microProvider = ref.read(microscopeCalibrationProvider);
    try {
